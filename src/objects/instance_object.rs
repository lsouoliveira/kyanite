...
pub fn create_instance_type(ob_type: TypeRef) -> TypeRef {
    Type::as_ref(Type {
        ...
        name: ob_type.borrow().name.clone(),
...
pub fn instance_tp_repr(
    ...
    if let KyaObject::InstanceObject(_) = &*object {
        let ob_type = callable.borrow().get_type()?;
        let repr = ob_type
            .borrow()
...
pub fn get_attr(
    ...
    if let Some(attr) = instance_object.dict.borrow().get(&attr_name) {
        return Ok(attr.clone());
    } else {
        let mut root_type = object.borrow().get_type()?;
...
pub fn instance_tp_set_attr(
    ...
    if let KyaObject::InstanceObject(obj) = &*object {
        obj.dict.borrow_mut().insert(attr_name, value);
        Ok(())
    } else {
        Err(Error::RuntimeError(format!(
            "The object '{}' is not a instance",
